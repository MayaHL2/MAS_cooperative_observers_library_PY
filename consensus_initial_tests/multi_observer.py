...
x, x_hat, x_hat_concatenated, A, B, C, y, y_hat, y_concatenated = initialize_random(size_agent, nbr_agent, nbr_step, range_initial_x, range_initial_x_hat)
A_noisy = noisy_parameters(A, std_noise = 0.1)
A_noisy = np.array(A) 

K = co.place(A_noisy, B, -1.5*np.abs(np.random.rand(size_agent)+0.75))
...
A_sys_noisy = np.kron(np.eye(nbr_agent),  A_noisy)
A_sys_concatenated_noisy = np.kron(np.eye(nbr_agent), A_sys_noisy)
...
K1 = step*np.kron(np.eye(nbr_agent), K1)
# print(np.shape(K1))
...
print(np.shape(C_sys), np.shape(C_sys_concatenated))
...
for i in range(nbr_step-1):
    ...
    x_hat_concatenated[:,i+1] = step*np.dot(A_sys_concatenated_noisy - step*np.dot(B_sys_concatenated, K_sys_concatenated) + step*np.dot(K1,C_sys_concatenated), x_hat_concatenated[:,i]) +step*np.reshape(np.dot(B_sys_concatenated, u_concatenated[:,:,i]), (-1, )) + step*np.dot(K2,np.dot(L_mn, x_hat_concatenated[:,i])) + x_hat_concatenated[:,i] 
